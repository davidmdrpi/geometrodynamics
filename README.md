...
## Is there a classical action that selects the branch? No — and the source becomes readable

*Pre-registered in `docs/history_action_prereg.md` (`a33a901`). Module
`geometrodynamics/bulk/history_action.py`; probe `history_action_probe.py`;
tests `tests/test_history_action.py`. Write-up: `docs/history_action.md`.*

**Five independent verdicts; the pre-registered headline is not printed.**
This round inherits the Hopf/spin-frame strand only — nothing in it depends on
the antipodal scalar BC, on `η`, or on quotient-versus-double-cover, since the
throat transports enter the reduced loop as the global `J² = −1` that cancels.

The candidate is the holonomy trace of the round-6 SU(2) holonomy,
`S_H = −½ Tr G = −cos(Ω/2) = −D/√(D²+N²)`. It does everything one could hope:
the derived closure set is exactly its **Morse–Bott critical manifold**, with
index `0` on `θ = 0` and `1` on `θ = π`, and its transverse stationary-phase
magnitude `|D|/|u×v|` **is** the positive coarea density of round 5, with
nothing tuned. It is still not an action.
**A — `HOLONOMY_TRACE_IS_A_STATIONARY_FUNCTIONAL_NOT_A_DERIVED_ACTION`.**
Closure holonomies at a common basepoint lie in the `U(1)` generated by `x`, so
`θ` composes additively (`3.3e-16`): `e^{iθ}` is a genuine holonomy and carries
the branch sign `sgn D`, but `∇θ = (u×v)/D ≠ 0`, so `θ` has **no critical
points at all** — off the closure set as well as on it, since a regular
critical point would need `x_p = −sec(γ/2)`, outside the sphere. `S_H` has the
critical set but is **not additive** (defect `1.805`), so `e^{iκS_H}` is not
the holonomy of any connection and `κ` is a free normalisation. *Scoped:* what
this closes is the class of real class functions of the holonomy alone — not
classical actions in general, which may carry `∫p dq − H dt`, detector terms or
boundary terms. Morse–Bott stationary phase gives
`A_π/A_0 = (M_π/M_0)e^{2iκ}e^{∓iπ/2}`, and the component masses are the
positive coarea masses of the two arcs, generically unequal
...
which is nowhere stationary, so `−cos θ` must be appended for its critical set.
...
## Independent composition selects no counting function (round 10)

The [preregistered joint-closure test](docs/joint_closure.md) asks what data a
joint history of two independently prepared triangles actually retains. The
joint closure locus is `Gamma_1 x Gamma_2`, the two differentials occupy
orthogonal tangent blocks, and the coarea Jacobian is
`|q_1||q_2| / |D_1 D_2|`, so the reference density is
`|D_1 D_2| / (|q_1||q_2|)` — a function of the **absolute product** alone.

**That sufficiency does not transfer to any other weight.** At `t_1 = t_2 = 1`,
`(1,2)` and `(sqrt2,sqrt2)` have the same signed product, and `(1/4,1)` and
`(-1/4,1)` the same absolute product; the reference density agrees to `1e-15`
on both pairs while the factorwise cubic `Phi(D) = D^2(1 - D/5)` separates them
by `0.1373` and `0.005`. Sufficiency of the reference measure is not a
...
(`3.0e-31`, and `2 sin(e_1) sin(e_2)` inside finite windows). Centrality does
not propagate to a rank-one joint condition, though: the differential of the
composed holonomy at a closed configuration is
`c_1 c_2 (x_1 d theta_1 + x_2 d theta_2)`, of **rank two** for non-parallel
axes (singular values `0.5, 0.5` in the reviewed configuration), so it
reproduces the two independent conditions and opposite phases do not cancel.
The rank-one defect belongs to the explicit scalar sum alone. What the holonomy
route gives is a joint *closure condition*, not a joint *weight*, and it
assumes a common quaternion frame that is not derived. The transaction modules
...
`NO_SELECTION_FROM_INDEPENDENCE`. Review corrections N23-N29 (excision cutoff, off-closure
sampling, a withdrawn rank-one claim, disconnected and narrow window gaps,
verdict schema, a mislabelled control) are recorded in
[the write-up](docs/joint_closure.md#review-corrections); none changes the
conclusion. The freeze's conditional P3 obstruction is
...
The public freeze `b78157a` preceded implementation. **49 tests and all 17
