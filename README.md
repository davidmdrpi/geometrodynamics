...
| Quark winding β = N·π/2 with N=466 | **Phenomenological (scope sharpened)** | `N = 2·n_part`, parity (Z₂) topological; `n_part = 233` is fit compensator absorbing the inter-generation hierarchy — now diagnosed as **dynamical** (QCD-RG): the quark hierarchy is irregular (neither power-law like leptons nor exponential like neutrinos), so a geometric closure integer can only compensate it (`quark_beta_*` probes, PRs #76, #80, #97) |
