...
points at all**. `S_H` has the critical set but is **not additive** (defect
`1.805`), so `e^{iκS_H}` is not the holonomy of any connection and `κ` is a
free normalisation. No single functional supplies both the magnitude and the
sign. Concretely `A_π/A_0 = e^{2iκ}e^{∓iπ/2}`: the magnitude is `1` for every
real `κ`, the ratio is real only when `4κ/π` is an odd integer (convention-
independent), the sign then alternates with `κ` mod `π` — and `κ = 1` gives
...
**C — `CLASSICAL_DETECTOR_RESPONDS_QUADRATICALLY`.** Measured under `φ ↦ cφ`,
all five null stresses of `source_audit.py` and the conserved mouth current
`Im(q*Aq)` have degree exactly `2.000000`; none is linear. And a quadratic
readout is not milder but **superquantum**:
`E = −2cos γ/(1+cos²γ)`, marginals still exactly `1/2`, and
`S_max = 8√2/3 = 3.7712 > 2√2`. Linearity is what keeps the model *at* the
quantum bound, and the matter sector does not supply it.
...
which is nowhere stationary. `−cos θ` must be appended for its critical set.
**E — `SOURCE_READOUT_SIGNALS_FUTURE_SETTINGS`.** The conditioned density is
*exactly* antipodally even (`3.4e-20`), so every **odd** source observable is
blind — `E[x·m|a,b] = 0`, `P(x·m>0) = 1/2` exactly. But every **even** one
signals (`E[(x·m)²]` spread `0.0103`), and by C every BAM coupling is degree 2,
hence even: the homogeneity that fails to give a linear readout is the one that
makes the source readable. For non-coplanar settings the supports are mutually
singular (total variation `1.0`) — a one-shot signal. Detector no-signalling
stands; **operational no-signalling to the past does not**. Declaring `x` gauge
would remove both the signal and round 5's physical source variable; that
collision is recorded, not resolved.

**Net: the round removes none of the three underived inputs and adds a fourth
(`κ`), plus a failed causality gate.** The geometry supplies the branch
holonomy as a label and the coarea density as a magnitude — from two provably
different functionals — and nothing yet makes either a probability.

```bash
python -m experiments.closure_ledger.history_action_probe   # 16/16
```
