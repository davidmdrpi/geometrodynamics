...
**The σ_z-readout capstone: the geometric object, identified and
classified (PR #241).** The measurement arc's closing question —
*identify the precise geometric object that would generate
σ_z-sensitive readout; classify it as derived geometry, additional
detector structure, or imported measurement freedom* — answered with
an exact three-way factorization (`docs/sigma_z_readout_capstone.md`).
Probe K (#238) fixed the meaning: the derived fiber U(1) dial is
GENERATED by σ_z and the derived σ_z Stern–Gerlach commutes with it —
the abelian trap, CHSH = 2 by commutativity; sensitivity to the dial
requires the Δk = 2 mixer L found (pure σ_x, 1/√6) and M validated
(Z₈-conserving with a winding-2 carrier), leaving one open item: the
carrier's geometry. **THE OBJECT IS AN ELLIPTIC MOUTH** — a quadrupole
(m = 2) deformation of the mouth's fiber circle, equivalently the mean
field of a winding-2 condensate: deforming R(χ) = R₀(1 + ε cos(2χ +
φ₀)) gives **⟨+1|H|−1⟩ = (2−√2)·ε·e^{iφ₀} EXACTLY** on the committed
N_χ = 8 fiber (linear in ellipticity, phase = orientation, both at
machine), with the multipole selection rule exact at O(ε) — only the
quadrupole connects the k = ±1 qubit. **THE PARITY HOME:** the #202
theorem's two branches re-derived on the committed bridge barrier —
odd modes: EXACT neck node (2e-10; the #221 committed near-neck law
re-read); even modes: finite neck amplitude (contrast 1e7) — the
carrier lives in the even sector the lepton ladder {1,3,5} is
parity-EXCLUDED from: apparatus and matter occupy complementary parity
sectors of one field. **THE POPULATION SCAN (honest negative):** no
committed mechanism populates the even sector coherently (lepton
ladder odd; #58 makes ±1 pairs; #223 dressing phase-blind; vacuum has
no phase) — M's n̄ ≈ 16 condensate (back-action re-read: 1→2.29,
4→2.60, 16→2.76) is APPARATUS, the Stern–Gerlach magnet of the
substrate. **THE PHASE IS THE DIAL:** the qubit restriction at
orientation φ is 0.408248(cosφ σ_x − sinφ σ_y) — |coefficient|
constant to 3e-12 (= 1/√6 = L's ledger), angle = −φ exactly, and CHSH
with the DERIVED detector is Tsirelson at EVERY orientation (4e-15):
the Bloch-sphere measurement freedom reduces to ONE physical U(1)
angle at the mouth — located, not eliminated. Superselection
reconciled on the carrier model: [H, K_total] = 0 exact, total-charge
distribution frozen (3e-16) while the reduced field carries winding
coherence — the #208 no-go untouched. **THE CLASSIFICATION:** operator
= DERIVED GEOMETRY; occupation = ADDITIONAL DETECTOR STRUCTURE;
orientation = the IMPORTED MEASUREMENT FREEDOM, reduced to one U(1)
and physically located. The #238 trilemma is factored, not evaded.
Scope: K/L/M lattice level (the #206 (x,χ) lattice); first order in ε;
n̄ re-read not re-derived; populating the condensate = the named
successor; pointer-phase stability = #209/#219
(`sigma_z_readout_capstone_probe`, PR #241).
