...
| `minimal_mixing_interaction_probe` (L) | **#238's missing interaction, tested directly instead of assumed away — it exists, it is unique, it works, and it costs charge.** **Uniqueness:** among mouth-localized fiber-angle potentials `λcos(2πmχ/N+φ)`, only **m = 2** connects the qubit (|⟨+1|V|−1⟩| = **0.4082**; m = 1, 3, 4 give **exactly 0**) — the qubit is `k = ±1` so the coupling must carry `Δk = 2`. The minimal missing interaction is the **second χ-harmonic at a mouth**: a **fiber-angle anisotropy**, a mouth not round in the fiber direction. **It is exactly the missing generator:** restricted to the qubit it is **pure `σ_x`**, coefficient **0.408248**, with `I`/`σ_y`/`σ_z` coefficients zero to **1e-16**. **It works with the DERIVED detector** (no transverse readout assumed): CHSH **2.000000 → 2.828427**, Tsirelson. *So #238's gap is not a no-go.* **COST 1 — CHARGE:** winding **is** charge (#42–#44) and the committed dynamics conserves it exactly (`‖[H,K]‖ = 6.6e-16`), but `‖[V₂,K]‖ = **1.414**` — the fiber U(1) protecting winding is *exactly* the symmetry the mixing term must break, so the same generator cannot both conserve the charge and rotate the qubit. **COST 2 — LEAKAGE:** the term drives population into `k = ±3`, so rotating the detector depletes the qubit. Treating leakage conservatively as detection inefficiency and applying `S > 4/η − 2`: a **loophole-free window exists** but is narrow — margin positive only for `|t| ≲ 0.6`, peaking at **S = 2.1349, η = 0.9704, margin +0.0130** — and **Tsirelson saturation is NOT in it** (at S = 2.828, η = 0.470 against threshold 6.52). Caveat: the leaked population sits in other winding channels and is in principle detectable, so the inefficiency reading is conservative, not forced. **Net:** testing beat assuming, but both costs are structural and the charge one is decisive — a quantitative case for relocating the Bell chain to the spin doublet, whose qubit is not the charge | THE_MINIMAL_MISSING_INTERACTION_IS_THE_SECOND_CHI_HARMONIC_AT_A_MOUTH_WHICH_IS_EXACTLY_PURE_SIGMA_X_ON_THE_QUBIT_AND_RESTORES_CHSH_TO_2_83_BUT_IT_BREAKS_WINDING_CHARGE_CONSERVATION_AND_ONLY_S_2_13_SURVIVES_THE_LEAKAGE_LOOPHOLE_FREE |
