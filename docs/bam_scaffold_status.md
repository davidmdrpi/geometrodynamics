...
| `detector_algebra_probe` (K) | **after #237, what remains is the detector marginals and the measurement algebra — and they are one question.** **The algebra is NOT the restriction:** the observables reachable by conjugating a fixed detector with the setting group span only a great circle, but the `*`-algebra they **generate** is all of `M₂(C)` (dim **4/4** — a product of two x–y observables already gives `σ_z`). What is restricted is **which self-adjoint elements are dialable**. **Docs vs code disagree:** the documents say the setting is "the fiber-frame rotation", which on the `k = ±1` qubit is `diag(e^{iθ},e^{−iθ})` (a z-rotation), but committed `measurement_sector_probe._rot` is `exp(−iθσ_y/2)` (a y-rotation, verified to **0.0e+00**) — and the fiber rotation **commutes exactly** with the `σ_z` winding detector (**0.00e+00**) while the y-rotation does not. **THE TRILEMMA:** max CHSH on the #206 singlet — fiber×`σ_z` (**both derived**) **2.000000**, dialable span **1**; fiber×`σ_x` (transverse assumed by #236/#237) 2.828427; y-rot×`σ_z` (the code) 2.828427. **The only pairing in which both ingredients are physically derived gives exactly the local bound — no Bell violation** — because the setting commutes with the detector and the dial does nothing. **The marginals follow the same fork:** #237's "identically zero marginals, immune to mixing" holds only for fiber×`σ_x`; under the other two the marginals are **|cos 2β|** exactly (up to **0.866**) — so that claim is **retracted as a general claim about BAM**, and #237's falsifiable content was contingent on the same undetermined choice the violation hangs on. **Both missing ingredients are the same requirement:** `σ_x` is off-diagonal between `k = ±1` and the y-rotation mixes those sectors, so BAM must supply **an operation or observable coherently connecting distinct winding sectors** — and imposing winding superselection gives max CHSH **2.000000** over 4000 draws, exactly local. Not a refutation: the violation is **located**, not overturned. Open: reconciling #208's charged-GHZ superselection with a Bell chain needing exactly the coherence it removes; whether relocating onto the transported-frame/spin doublet (#192/#197 Berger-squash) avoids it | THE_ALGEBRA_IS_FULL_BUT_THE_DIALABLE_SET_IS_NOT_AND_THE_ONLY_PAIRING_OF_A_DERIVED_SETTING_WITH_A_DERIVED_DETECTOR_GIVES_CHSH_EXACTLY_TWO_SO_THE_VIOLATION_RESTS_ON_WINDING_SECTOR_COHERENCE_THAT_IS_NOWHERE_DERIVED |
