...
| `admissible_correlation_tables_probe` (J) | **which tables the encoding admits — not just how big CHSH gets.** #236 probed the CHSH *maximum*; this characterizes the whole table. **Level A:** the representation `E_xy = ⟨ψ|A_x⊗B_y|ψ⟩` admits **exactly the unit-vector Gram matrices**, equivalently the Tsirelson–Landau–Masanes body — verified both ways (fit vs predicate **60/60**; 4000 Gram-built tables never violate, min slack **+2.6e-04**). Bodies: local **66.8%** of the cube, quantum **92.6%**, no-signaling 100%; PR box at slack **−π**. **Level B — what BAM implements is strictly smaller.** Read off the lattice: the pair state's T-matrix x–y block is **exactly `c·I₂`** (deviation ≤ 3.1e-15) with **`c = −sin 2β`**; with #236's U(1) settings the achievable tables are **exactly `E_xy = c·cos(θ_x − φ_y)`**. Coverage: **0.0%** at maximal preparation (a measure-zero 3-parameter surface, **33.7%** of it exactly on the Tsirelson boundary — which is why #236 saw saturation), **55.2% ± 2.0%** with `β` free (2000-sample run 54.6% ± 1.1%), against **100%** for general `d = 3`. **So the bridge preparation `β` is the one knob buying the missing dimension.** Explicit unreachable quantum table recorded (TLM slack +0.740, `d=3` residual 4.3e-09, BAM residual 4.6e-01). **And the marginals vanish identically** (max |⟨A_θ⟩| = **0.000e+00**) though the state carries `cos 2β` — so no biased-marginal behavior is representable at all; the restriction is in the readout, the same place #236 found the ceiling. **Method note:** generic optimizers got membership wrong in *both* directions (L-BFGS-B 48.3%, Nelder-Mead 63.3%) — a failed fit is not proof of non-representability; replaced by an exact 1-D branch/bisection test validated by 100% recovery on family-built tables. Scopes #236's "exactly sufficient" to the CHSH maximum. Falsifiable: BAM cannot reproduce biased marginals or the ~45% of tables needing non-coplanar directions | THE_REPRESENTATION_ADMITS_EXACTLY_THE_UNIT_VECTOR_GRAM_TLM_BODY_BUT_BAM_IMPLEMENTS_ONLY_C_COS_THETA_MINUS_PHI_WHICH_IS_MEASURE_ZERO_AT_MAXIMAL_PREPARATION_AND_ABOUT_HALF_THE_BODY_WITH_BETA_FREE_AND_HAS_ZERO_MARGINALS |
