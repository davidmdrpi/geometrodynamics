...
| `rp3_spin_structure_on_223_ring_probe` (F) | pushes the spin-structure machinery through the **#223** ring — the one network Probe E found with deck class 1, where `W` and the B2 spin structure genuinely coincide. There the `end` (source-point) parity **is** `W`, and the predicted moding shift is **measured**: the two `W` sectors interleave with fractional offset → **0.5** (0.529, 0.504, 0.499, 0.506, 0.498), confirming on an actual construction a claim that was only conditional. But with the labels identified, B–D apply to `W` and **still nothing selects** (identical `\|det\|`, `h=0`, only an `η` phase apart). **Structural finding: #223 couples the labels but has NO mouth doublet; #224 has the doublet but deck class 0 — the phenomenon and the coupling NEVER co-occur**, so RP³ spin-structure data cannot select the exchange-freeze sector even in principle | ON_THE_223_RING_THE_LABELS_COINCIDE_AND_THE_MODING_SHIFT_IS_REAL_BUT_NOTHING_SELECTS_THE_TWIST_AND_THE_EXCHANGE_FREEZE_LIVES_ON_THE_OTHER_RING |
| `freeze_deck_parity_obstruction_probe` (G) | **upgrades Probe F's observation to a theorem, and withdraws its recommended next step.** A cyclic ring of `N` throats has `N` exterior π-arcs, so **deck class = `N mod 2`** (class 1 ⟺ `N` odd); and the freeze needs the cyclic translation `S^N = W` to have **no real eigenvalue**, which for `W = −1` holds **iff `N` is even**. So freeze ⟺ `N` even, deck class 1 ⟺ `N` odd — **mutually exclusive by parity, for every `N`**. Measured on real `N`-throat rings (#224's construction generalised): `N=2` twisted gap 1.995e-11 **frozen**, `N=3` 3.953e-01 **not frozen**, `N=4` 5.664e-12 **frozen** — `N=3` being the decisive case, the smallest ring with both several throats and deck class 1. **#231's recommended construction therefore cannot exist**, and the freeze sector is cut off from RP³ spin-structure data permanently rather than contingently. Open: the law is derived for CYCLIC rings only — non-cyclic networks (junctions, multi-cycle trees) are not covered | THE_FREEZE_REQUIRES_EVEN_N_AND_DECK_CLASS_ONE_REQUIRES_ODD_N_SO_THEY_ARE_MUTUALLY_EXCLUSIVE_BY_PARITY_AND_THE_231_RECOMMENDED_CONSTRUCTION_CANNOT_EXIST |
