...
| `bam_spinor_spectrum_effective_action_probe` | computes BAM's actual Pin⁻ spinor instead of asserting its statistics, and **corrects the row above**. The holonomy composes with the intrinsic B2 spin structure (`T²=−I`, `η=−1`), which **swaps the spinor's moding** relative to the scalar; the two sign flips cancel and `ΔE=+π/(4C)` for the spinor too — **energetics favour untwisted in both sectors**. What puts matter in the twisted sector is the **index**: `dim ker D = 1`, `det D = 0` there vs a `π/C` gap and `det D = 2` untwisted — #195's Atiyah–Singer mechanism from the network side. Selection is **two mechanisms** (energy for the bosonic QCD sector, index for the fermionic matter sector). Spectrum via the repo's SUSY square root, `O(1/n²)`; method note: a central-difference Dirac operator is unusable (doubling gives a spurious second zero mode) | THE_BAM_SPINOR_SWAPS_THE_MODING_SO_ENERGETICS_FAVOUR_UNTWISTED_IN_BOTH_SECTORS_AND_MATTER_IS_SELECTED_BY_THE_ZERO_MODE_INDEX_NOT_BY_ENERGY | THE_TWIST_SECTOR_IS_SELECTED_ENERGETICALLY_AND_THEN_TOPOLOGICALLY_FROZEN_AT_NUCLEATION_SIGN_RULE_CORRECTED_BY_THE_SPINOR_PROBE |
