...
scaffold. The scaffold (PR #49; **note:** the probe module
`bam_effective_action_scaffold_probe` is referenced throughout this
document but is **not present in the repository** and does not appear
anywhere in git history or the archived tarballs — its results are
recorded here and in `docs/topological_discrete_sector_research_plan.md`
but are not independently reproducible) proposed
...
|---|---|---|
| `bam_effective_action_scaffold_probe` **(module absent — see note above)** | B1–B5 (map) | SCAFFOLD_WITH_BARRIERS |
| `topological_discrete_sector_probe` | B1, B2 | PROMOTION_SUCCEEDS |
| `hard_wall_boundary_verification` | B3 | HARD_WALL_DERIVED |
...
| `quartic_vertex_bounded_interaction_probe` | extends the #137 cubic ledger to the quartic vertex + audits whether the matter interaction is bounded below. FACTORISES: V_4=λ_4·[∫_{S³}Y_{l1}Y_{l2}Y_{l3}Y_{l4}dΩ]·[∫ψ_kψ_lψ_mψ_n dr*]. QUARTIC ANGULAR RULE (DERIVED): nonzero only if (a) l1+l2+l3+l4 EVEN — the SAME antipodal Z₂ as the cubic (#137, x→−x C-swap #63) — AND (b) common SO(4) channel ∃L∈[|l1−l2|,l1+l2]∩[|l3−l4|,l3+l4]. Verified exactly (odd-Σl→0); Z₂ parity persists cubic→quartic. POSITIVE SELF-OVERLAP (DERIVED): g_4=∫ψ_k⁴ dr*>0 manifestly (integral of 4th power; 1.03,1.02,…). BOUNDED INTERACTION ⟹ STABLE VACUUM: single-mode V(a)=½ω²a²+(λ_3 g_3/6)a³+(λ_4 g_4/24)a⁴ has a⁴ coeff λ_4 g_4/24>0 (g_4>0, λ_4>0) ⟹ V→+∞ as |a|→∞ for ANY cubic ⟹ bounded below, stable vacuum (cubic only tilts, never unbounds; verified V(±10⁴)>0 up to λ_3=200). BOUNDEDNESS = MEASURE CONVERGENCE (#122): a bounded-below action is exactly the condition for ∫Dμ e^{−S} to converge (non-perturbative, #122), so the positive quartic is required by, not added to, the measure's existence — extends the stability thread (#130 stable spectrum, #136 unitary self-energy, #138 bounded vacuum). Open: coupling magnitudes λ_3,λ_4 input (sign λ_4>0 from #122), quintic/higher vertices, S_BAM generation | QUARTIC_VERTEX_LEDGER_Z2_SELECTION_POSITIVE_OVERLAP_BOUNDED_STABLE_VACUUM |
| `antipodal_matter_interaction_synthesis_probe` | capstone of the antipodal matter-interaction arc (#129–#138): re-verifies a keystone from each member together (cross-arc consistency) + organises the arc into two threads from one postulate + epistemic ledger. KEYSTONES (re-run together, all consistent): Y_l parity [1,−1,1,−1]=(−1)^l (#129); antipodal fundamental real ≈1.17 vs absorbing complex ≈1.89−1.16i (#130); kernel reciprocity ~1e-14, real poles (#135); lightest-mode Im Σ≈0 (stable, #136); ∫ψ⁴=1.03>0 (bounded vacuum, #138). THREAD A — antipodal Z₂ (−1)^l: C-swap inversion x→−x (#63) carrying Y_l→(−1)^l Y_l fixes BC (#129), grades exchange kernel (#135), selects cubic (#137) + quartic (#138) vertices (Σl even). THREAD B — unitarity/stability: unitary mirror (#129) ⟹ real stable spectrum (#130) ⟹ unitary reciprocal propagator (#135) ⟹ unitarity-preserving self-energy + stable lightest mode (#136) ⟹ bounded-below vacuum (#138 = #122 measure convergence); stable at every order. ONE POSTULATE TWO FACES: the real l-parity BC (#129) IS both the Z₂ grading and the unitary mirror. EPISTEMIC LEDGER: DERIVED (given the antipodal BC) — Z₂ selection structure + unitary stable propagator/self-energy/vacuum; POSTULATED — the antipodal identification (#128, self-consistent not forced); INPUT — coupling magnitudes λ_3,λ_4 (sign λ_4>0 from #122); OPEN — S_BAM vertex generation, higher loops/vertices, scale (#133), flavor (#134) | ANTIPODAL_MATTER_INTERACTION_SYNTHESIS_Z2_SELECTION_AND_UNITARY_STABILITY |
| `s_bam_vertex_generation_probe` | closes the #137–#139 open item (the interaction vertices were modelled, not derived). VERTICES = TAYLOR COEFFICIENTS OF S_BAM: S_BAM[φ_cl+φ]=S_cl+S_2+S_3+S_4+…, S_n=(1/n!)∫(δⁿS/δφⁿ)φⁿ — S_2 the #116 determinant/#135 propagator, S_3=(g/3!)∫φ³√g ⟹ ∫ψ_kψ_nψ_m (#137), S_4=(λ/4!)∫φ⁴√g ⟹ ∫ψ⁴ (#138); geometric (non-quadratic) S_BAM generates the tower, a free quadratic action none. SELECTION RULE = ANTIPODAL WARD IDENTITY: the S_BAM measure carries the Diff(S¹)⋉U(1)⋉Z₂ quotient (#74), whose Z₂ is the C-swap A:x→−x (#63/#128); under A a mode amplitude transforms a_l→(−1)^l a_l, so a vertex picks up (−1)^{Σl} and is A-invariant ⟺ Σl even. S_BAM A-invariant ⟹ every vertex Σl-even = the #137/#138 rule as a WARD IDENTITY (verified: A-invariance matches explicit S³ odd-Σl vanishing). QUARTIC SIGN = MEASURE CONSISTENCY: ∫Dμ e^{−S} exists (reflection-positive ⟹ unitary kernel #135; convergent #122) ⟺ S bounded below ⟺ λ_4>0 (#138), realised by ∫ψ⁴>0 — the positive sign required not chosen. LEDGER: DERIVED — vertex existence (action expansion), Σl-even selection (antipodal Ward identity), positive quartic sign (measure consistency #122); INHERITED — coupling magnitudes g,λ (the action's higher derivatives; κ₅²/Λ₅ scale #133). ⟹ vertex STRUCTURE generated; only MAGNITUDES inherit #133. Open: exact S_BAM form, coupling magnitudes, scale (#133), higher vertices, flavor (#134) | S_BAM_VERTICES_FROM_ACTION_EXPANSION_Z2_WARD_SELECTION_MEASURE_POSITIVITY |
| `gauge_matter_coupling_probe` | joins the gauge sector (U(1)_Hopf photon 1/q², #42–#44) to the matter sector (antipodal cavity modes, #129–#140) at the throat. (QFT on the fixed classical throat geometry, not quantum gravity.) MINIMAL COUPLING: matter of Hopf charge c₁ (|c₁|=1, #58/#74) via D_μ=∂_μ−ic₁A_μ, vertex c₁∫A_μ j^μ. C-SWAP = INVERSION × CHARGE CONJUGATION: antipodal map A:x→−x (#63) acts at once on matter harmonics (Y_l→(−1)^l Y_l, #129/#140) and Hopf charge (c₁→−c₁, #63) — one operation, two effects ⟹ the throat is the particle↔antiparticle (C) surface (#63/#64). GAUGE VERTEX INHERITS ANTIPODAL Z₂ SELECTION: photon-matter-matter angular part = triple overlap ∫Y_{l_γ}Y_{l₁}Y_{l₂} (cubic-vertex structure #137 + gauge leg); antipodal invariance (#140 Ward identity) ⟹ Σl=l_γ+l₁+l₂ even (verified: odd-Σl forbidden). U(1) CHARGE CONSERVED AT THE THROAT: unitary mirror (#129, zero net matter flux) conserves charge flux; C-swap flip ⟹ outgoing charge re-emerges as conjugate ⟹ Σc₁=0 (#58) — the gauge face of the mirror. COUPLING STRENGTH = α: structure (covariant derivative, Σl-even vertex, charge conservation, C-surface) derived; strength = EM coupling α (the '137 problem', #105) — universal INPUT residual. Open: α (#105/#108), EM normalisation, higher gauge vertices, scale (#133), flavor (#134) | GAUGE_MATTER_COUPLING_MINIMAL_Z2_SELECTED_CHARGE_CONSERVED_ALPHA_INPUT |
| `gauge_ward_identity_probe` | audits the consistency of the #141 gauge–matter coupling — current conservation, the Ward–Takahashi identity, photon masslessness — and ties them to the unitary mirror (#129). (QFT on the fixed classical throat, not quantum gravity.) NOETHER CURRENT: global U(1)_Hopf ⟹ j^μ=i(ψ*∂^μψ−…), ∂_μ j^μ=0; stationary mode ⟹ ρ=2ω_n|ψ_n|² static. CURRENT CONSERVED AT THE THROAT: antipodal modes REAL (#135) ⟹ radial charge current j^r∝Im(ψ_n*∂_rψ_n)=0 EXACTLY (verified, all modes) — no charge flux, charge static/conserved = the zero-flux unitary-mirror property (#129). ABSORBING THROAT BREAKS IT: complex quasinormal modes (#130) carry j^r≠0 (verified ≈0.014 at throat) — charge leaks into horizon ⟹ current conservation fails ⟹ gauge invariance broken ⟹ gauge invariance REQUIRES the antipodal throat. WARD–TAKAHASHI: q_μ Γ^μ=S⁻¹(p_out)−S⁻¹(p_in) ties the gauge vertex (#141) to the matter inverse propagator (#135) — gauge coupling fixed by matter dynamics (Γ^μ=∂S⁻¹/∂p_μ normalises vertex to charge). PHOTON MASSLESSNESS: transversality q_μ Π^μν=0 ⟹ no photon mass ⟹ the 1/q² photon (#42–#44) protected. ONE POSTULATE BOTH: current conservation + Ward identity + masslessness all follow from the unitary antipodal throat (#129) — same real/self-adjoint/zero-flux structure as the stable spectrum (#130), unitary propagator (#135), stable self-energy (#136), bounded vacuum (#138); gauge invariance = the gauge face of the unitary mirror, not an extra assumption; only α (#105) input. Open: α (#105/#108), higher-order Ward identities, running of α, scale (#133), flavor (#134) | GAUGE_WARD_IDENTITY_CURRENT_CONSERVED_PHOTON_MASSLESS_FROM_ANTIPODAL_MIRROR |
| `alpha_normalization_ledger_probe` | consolidating ledger for the EM coupling normalisation α (the strength left input by #141/#142) — parallel to the bulk-scale ledger (#133). (QFT on the classical throat, not quantum gravity.) HOW α ENTERS: A_EM=α·ℏc/2 (#105), vertex strength ∝c₁²α (#141), Schwinger anomaly a=α/2π (#74) — a single dimensionless number. CHARGE QUANTUM DERIVED: |c₁|=1 integer Hopf number (#58/#74), charge quantisation topological, charge unit geometric. 1/2π MEASURE DERIVED: in a=α/2π the 2π is the closure-quantum loop measure (#74); BAM fixes the 1/2π, only α is the input prefactor. RUNNING DERIVED, VALUE NOT: RG flow (vacuum polarisation, transverse by #142 Ward identity) derived — BAM derives HOW α runs; boundary value α(μ_0)≈1/137 input — not WHERE it starts (#105). VALUE = ONE EM RESIDUAL (137 problem): fit-independent scan of α⁻¹=137.036 vs closure numbers (2π, k₅, 50π) finds NO clean match — near-misses (50π−20=137.08, 4·k₅²+37=137.0) each need ad-hoc additive O(20–37) term (fits not derivations, #107/#108 failure mode); α plausibly irreducible like √σ/m_e (#108). EM sector contributes exactly ONE residual, the value α ∈ {n_part, √σ/m_e, ε, α} (#104). Open: the value α (137 problem), EM normalisation, scale (#133), flavor (#134) | ALPHA_NORMALIZATION_LEDGER_CHARGE_QUANTUM_AND_2PI_MEASURE_DERIVED_VALUE_ONE_RESIDUAL |
...
| `glueball_closed_flux_loop_probe` (intercept corrected by #229) | glueballs = pure-confinement benchmark (closed flux loops, no quark/flavor input); BAM orientable ground √(4πσ)≈1.50 GeV (3.5√σ) benchmarks lattice 0++ (4.1√σ) to ~13%; closed-string glueball Regge slope = half the meson; BAM non-orientable Möbius sector ⟹ extra glueball tower (half-int modes, +πσ in M²) interleaving the orientable one (≈2× states); legitimate vs lattice not experiment (glueballs unobserved) | GLUEBALL_SCALE_BENCHMARKS_LATTICE_MOBIUS_TOWER_IS_BAM_TOPOLOGY |
...
| `nonorientable_er_link_z2_probe` | promotes the antipodal Z₂ from a global parity of one throat (#139 Thread A) to a **Z₂ GAUGE FIELD** over the ER network (#206–#224, where orientation was an inert decoration): link variables, gauge orbits, Wilson loops, only holonomies physical. Gauge invariance exact on the real 1114-site #224 ring (drift 0.00e+00); **a twisted link freezes mouth exchange** (Δω ratio 2e-11…5e-11, T_exchange 1833→3.8e+13) as a **theorem** — `S²=W` ⟹ every level exactly degenerate (7.1e-12); the twisted loop IS the repo's own Möbius sector (`make_glueball_ring`/`make_mobius_tube`); quotient revival halves to `πR` with twist-set sign. **Corrected by measurement:** no factor-of-4 energy advantage (energy conserved; `2 m_e c²` threshold untouched); the "new" vertex rule is a tautology. `b₁` falsifier does not fire on the physical graph | ER_LINK_TWIST_IS_A_Z2_GAUGE_FIELD_THAT_FREEZES_MOUTH_EXCHANGE_AND_HALVES_THE_QUOTIENT_REVIVAL_BUT_LEAVES_THE_NUCLEATION_THRESHOLD_UNTOUCHED |
| `er_link_twist_surgery_probe` | tests the #227 §8.1 conjecture that #207's surgery law gains `+π·[n_twist mod 2]`: **refuted** — outer-bridge twists move the pair phase by ≤2.3e-13, odd-`n_twist` rows miss `π` by ≥3.09 rad; the corrected law is the original `φ₁₄=φ_a+φ_b+φ_c`. Algebraic reason: the phase is `arg(c_mp/c_pm)`, a ratio over branches traversing the same links. The twist is still physical (spectrum moves ~2.5e-02); its only effect is **cycle interference**, dying with cycle length and scaling with the exterior hopping — #227's `b₁` principle confirmed dynamically | THE_SURGERY_COMPOSITION_LAW_HAS_NO_Z2_TERM_THE_PAIR_PHASE_IS_A_RATIO_AND_THE_TWIST_ACTS_ONLY_AS_CYCLE_INTERFERENCE |
| `mobius_identification_quantitative_probe` | is the Möbius identification quantitative? **Partially.** `ΔM² = (½)×(2πσ)`: the ½ is derived (three independent implementations, all exactly ½), topological (`O(1/M²)` exactly, circumference-independent to 1.4e-11) — #100 had only asserted it; the `2πσ` is an imported closed-string relation. **And #100's common intercept is unjustified:** antiperiodic moding shifts the zero point by `+π/(4C)` per polarization (two regularizations, 4.1e-07), scaling as `1/C` — the Möbius ground state is **not** `M₀²+πσ`. Scope: glueballs only; the #103/#109/#114 baryon search table is built from `2√σ` and stands | THE_MOEBIUS_IDENTIFICATION_IS_QUANTITATIVE_IN_ITS_TOPOLOGICAL_HALF_THE_SCALE_IS_INHERITED_AND_THE_MOEBIUS_INTERCEPT_IS_AN_OPEN_CORRECTION |
| `twist_sector_selection_probe` (sign rule corrected by the spinor probe) | what selects the twist sector (the arc's last open item): **energetics select, topology freezes.** Sectors differ by `π/(4C)` per d.o.f. (1e-12). `W` cannot relax: changing it drives the link amplitude through zero, severing the cycle (`b₁`:1→0) — the #175 gate; `W` is exactly deformation-invariant (200 re-weightings, zero-mode gap never closes). **Its "sign = statistics" rule is superseded** — see the next row |
| `rp3_spin_lift_probe` (A) | the spin-lift ledger: `J = −I₄` is in SO(4) (RP³ orientable); the only lifts `J̃₊=(−1,1)`, `J̃₋=(1,−1)` **both square to +1**, differing by the covering kernel — the spin structures are distinguished by *which SU(2) factor* carries the −1, not by `J̃²` (refuting the shortcut behind the `η=±1` bookkeeping). BAM's `σ` is orientation-**preserving** with `σ²=J`, and `T=iσ_y` gives `T²=J̃₊`: **B2 selects a lift**. But B2 ∈ `H¹(RP³;Z₂)` and `W` ∈ `H¹(Γ;Z₂)` are **independent**, so the `W·η` composition rule is unjustified unless the network cycle generates `π₁(RP³)` | BOTH_LIFTS_SQUARE_TO_PLUS_ONE_B2_SELECTS_J_PLUS_VIA_T_SQUARED_AND_B2_IS_INDEPENDENT_OF_THE_NETWORK_HOLONOMY_SO_THE_COMPOSITION_RULE_IS_UNJUSTIFIED |
| `rp3_dirac_eta_probe` (B) | the genuine first-order RP³ Dirac spectrum from the analytic S³ eigenspinors (no finite differencing ⟹ no doubling). The `±` branches carry **opposite** antipodal parity at the same `\|λ\|`; projection keeps exactly half the modes per level. **`η(0)=±1/4` exact** (vs convergent mode sum, 2e-19), **`h=0` for both** (the 1D 'zero mode' was an artefact), `ζ'_{D²}(0)=0.2189594807`, **identical `\|det D\|`** — the sectors differ only by a phase `π/4` apart. **Retires the energetic route exactly** | THE_RP3_SPIN_STRUCTURES_HAVE_h_EQUALS_ZERO_AND_IDENTICAL_DETERMINANT_MODULUS_AND_ARE_DISTINGUISHED_ONLY_BY_ETA_EQUALS_PLUS_MINUS_ONE_QUARTER |
| `bulk_aps_spin_structure_probe` (C) | `Y³=RP³=L(2,1)`, `X⁴`= D²-bundle over S² of even Euler number (spin); APS boundary defect `∓1/8`; `Ω₃^Spin=0` ⟹ **both spin structures extend**, so inflow does **not** select. `Ω₂^{Pin⁻}=Z₈` with `ABK(RP²)=±1` **forces paired mouths to carry opposite Pin⁻ structures** — deriving the `ThroatDefect` assertion, but only as a *relative* rule | BOTH_SPIN_STRUCTURES_EXTEND_SO_INFLOW_DOES_NOT_SELECT_BUT_ABK_FORCES_THE_TWO_MOUTHS_OF_A_THROAT_TO_CARRY_OPPOSITE_PIN_MINUS_STRUCTURES |
| `twist_sector_einstein_dirac_probe` (D) | precondition met (C). `ΔE_vac = 0` **exactly** (identical `\|spec\|`) and `Δ⟨T_AB⟩ = 0` **pointwise** by homogeneity of the round RP³ ⟹ identical radial semiclassical system, regularity, action modulus and stability; only the action **phase** differs, and a phase does not source `⟨T_AB⟩`. Not done: the non-homogeneous Tangherlini exterior | BACK_REACTION_IS_EXACTLY_DEGENERATE_BETWEEN_THE_TWIST_SECTORS_SO_GRAVITY_DOES_NOT_SELECT_EITHER_AND_ONLY_THE_ETA_PHASE_DISTINGUISHES_THEM |
| `network_cycle_pi1_class_probe` (E) | **decides whether B–D apply at all.** A loop in `RP³=S³/{±1}` is classified by lifting to `S³`: closes ⟹ class 0, ends antipodal ⟹ class 1 (generator). Read from the constructions: #224 sets `Ltot = 2*_D_CH+4*_SIG_M+2*_LARC` with `_LARC=3.14` — **two** π-arcs, total 2π, **lift closes, deck class 0 — NOT the generator**; #223 has **one** π-arc, deck class 1 — **is** the generator. And the ambient is not RP³: a wormhole is a 1-handle, so `π₁ = π₁(exterior) * Z^n`, and the #224 cycle's class lives in the **free (handle) part**. Robust to reading the exterior as `S³` or `RP³`. **Consequence: Probes B–D target the deck Z₂ and therefore do NOT bear on the #224 network twist** — a scoping correction, not a refutation | THE_224_CYCLE_DOES_NOT_GENERATE_PI1_RP3_ITS_DECK_CLASS_IS_TRIVIAL_AND_ITS_HOLONOMY_LIVES_ON_A_HANDLE_GENERATOR_SO_PROBES_B_TO_D_DO_NOT_BEAR_ON_IT |
| `rp3_spin_structure_on_223_ring_probe` (F) | pushes the spin-structure machinery through the **#223** ring — the one network Probe E found with deck class 1, where `W` and the B2 spin structure genuinely coincide. There the `end` (source-point) parity **is** `W`, and the predicted moding shift is **measured**: the two `W` sectors interleave with fractional offset → **0.5** (0.529, 0.504, 0.499, 0.506, 0.498), confirming on an actual construction a claim that was only conditional. But with the labels identified, B–D apply to `W` and **still nothing selects** (identical `\|det\|`, `h=0`, only an `η` phase apart). **Structural finding: #223 couples the labels but has NO mouth doublet; #224 has the doublet but deck class 0 — the phenomenon and the coupling do not co-occur on either of these two rings**. *Scope correction (PR #233): that is an observation about these two rings only — a third, two throats with quarter-circle arcs, carries the doublet, the freeze **and** deck class 1 at once, so RP³ spin-structure data is not cut off from the freeze sector after all. It still selects nothing there.* | ON_THE_223_RING_THE_LABELS_COINCIDE_AND_THE_MODING_SHIFT_IS_REAL_BUT_NOTHING_SELECTS_THE_TWIST_AND_THE_EXCHANGE_FREEZE_LIVES_ON_THE_OTHER_RING |
| `freeze_deck_parity_obstruction_probe` (G) | **claimed a parity obstruction; the claim is RETRACTED here, and the geometry it ruled out EXISTS.** What stands: the freeze needs the cyclic translation `S^N = W` to have **no real eigenvalue**, which for `W = −1` holds **iff `N` is even** — argued (roots of unity), measured (`N=2` twisted gap 2.2e-12 **frozen**, `N=3` 4.0e-01 **not frozen**, `N=4` 1.7e-12 **frozen**) and now verified at operator level (`‖[H,S]‖ < 1e-8`, `S^N = W` exactly). What was wrong: **deck class = `N mod 2`** assumed every exterior arc has length π, which forces all `N` throats to share one antipodal mouth pair. Probe E's actual criterion is **deck class = (Σ arc lengths)/π mod 2**, a function of **mouth placement**, not of `N`. So the two are **independent**: a ring of **two throats whose mouth pairs are a quarter circle apart** (arcs π/2, total π) has deck class **1**, a mouth doublet, and freezes — twisted gap **8.5e-13**, by the same `S² = W` mechanism. #224's coincident-mouth configuration is the degenerate case. Other evasion classes computed rather than asserted: one throat with `D > 0` gives a **nondegenerate interior ladder** (no doublet, no freeze — #231's item, upheld but now by calculation); unequal arcs, unequal channels or an internal bump each **destroy** the freeze even at even `N` (it needs the cyclic symmetry *exactly*); a **non-cyclic theta network** (`b₁ = 2`) does not freeze in any twist sector, its degeneracies being `S₃` permutation degeneracies present untwisted too. Net effect: **reopens** the line of attack — Probes B–D now apply to a network that actually freezes, and still find nothing selecting the twist | THE_FREEZE_NEEDS_EVEN_N_BUT_THE_DECK_CLASS_IS_SET_BY_MOUTH_PLACEMENT_NOT_BY_N_SO_THEY_ARE_INDEPENDENT_AND_A_TWO_THROAT_RING_WITH_QUARTER_CIRCLE_ARCS_CARRIES_BOTH |
| `single_throat_mouth_doublet_probe` (H) | **can ONE throat carry a mouth doublet at deck class 1? YES to the states, NO to the doublet that matters.** #233 checked only the interior channel LADDER; but #224's doublet is a MOUTH doublet and one throat already has two mouths. Put a barrier mid-channel and each sub-well abuts one mouth: localization **0.978 → 0.999**, splitting tunable **1.3e-01 → 5.8e-03**, deck class **1** throughout — so #233's "no doublet on one throat" was **too narrow**. But the **twist has no purchase**: the two wells are joined by two paths and only the exterior one carries the holonomy, so `split = 2|t_bump + W·t_arc|` — and extracting both sectors gives `|t_arc| = 2.0…2.9e-6` **independent of the bump height** (the two-path picture confirmed, not assumed) while `|t_bump|` falls **47×**; the exterior path crosses **two** mouth barriers, so the interference never balances (ratio > 0.999 throughout). The resonant two-basin doublet (avoided crossing at `D ≈ 8.25`, gap 3.2e-3) moves only ~10%. **The mechanism:** the freeze is `S^k = W` for a cyclic **translation**; a one-throat ring's two mouth walls both face the same interior so they **mirror**, giving a **reflection** `R` with **`R² = +1` in both twist sectors** — no holonomy-dependent square, ever. Same ring built both ways (identical lengths, basin profiles, deck class, two mouths; only the wall orientation differs): mirror `‖[H,S]‖ = 4.9e+01`, `‖[H,R]‖ = 0`, **no freeze**; translate `‖[H,S]‖ = 0`, `S²=W`, twisted sector **FROZEN at 3.0e-12**. So the governing integer is the **order of the cyclic translation**, which equals the throat count *because* of wall orientation — two throats are the minimum for a translation to exist. Consequence: #234's twist switch **cannot** move onto a one-throat network — not for want of a doublet, but for want of anything for the twist to act on | ONE_THROAT_GIVES_TWO_MOUTH_LOCALIZED_STATES_AT_DECK_CLASS_ONE_BUT_NEVER_A_TWIST_CONTROLLED_DOUBLET_BECAUSE_ITS_TWO_MOUTH_WALLS_MIRROR_RATHER_THAN_TRANSLATE_SO_THE_RING_CARRIES_A_REFLECTION_WITH_R_SQUARED_PLUS_ONE |
| `tsirelson_ceiling_probe` (I) | **what caps BAM's nonlocality at Tsirelson — and the answer is not the geometry.** #206 escapes Bell by spending the bridge, reaching CHSH = 2√2; but PR boxes are no-signaling and reach the algebraic **4**, and nothing in the repo said what stops BAM. **The ceiling is real:** max over all 8 gluing holonomies and all preparations = **2.828427125**, excess **+8.9e-16** — never exceeded, so BAM does not predict super-quantum correlations. **The settings are exactly sufficient:** BAM's fiber-frame rotation is `diag(e^{iθ},e^{−iθ})`, a one-parameter **U(1)** not SU(2), sweeping only the x–y plane — and that is enough because CHSH's optimum lies in a plane (singlet T-matrix x–y block singular values (1,1) → 2√2, identical at grid n = 25/49/97/193). **But locality is NOT the binding constraint:** the mouth projectors commute to **4.7e-113** (the committed bridge is a **link in H between distinct sites**, not an identification of degrees of freedom — #206's prose reads otherwise), yet dropping commutativity and the tensor split entirely still gives **2.828427095**, because `(B₀+B₁)² + (B₀−B₁)² = 4I` for any dichotomic observables (residual 5.3e-15) and Cauchy–Schwarz caps CHSH without using locality anywhere. **So the residue of #232's "quantization is imported" is exactly one identity, `B² = I`** — the geometry supplies the state (fidelity 1.000000000000), the nonlocality (raw B amplitude 3.7e-01 with the handle, 4.1e-16 without; local strategies cap at 2) and the settings, but **not the ceiling**. Naming it is the progress; deriving it is not attempted. **Plus:** #206's T6 no-signaling test is a partial-trace identity true of every bipartite state, so it tests nothing — tested on the lattice, no-signaling is **equal-time only**: the always-on handle shifts Bob's marginal by 2.3e-02/1.0e-01/**2.3e-01** at dt = 0.5/2/8, scaling with the coupling and vanishing when cut. Open: an argument that a classical throat readout must be two-valued (the Pin⁻/`T²=−1` machinery does not currently address the readout algebra); the handle must be made dynamically inert at measurement time | BAM_REACHES_TSIRELSON_EXACTLY_AND_NEVER_EXCEEDS_IT_BUT_THE_CEILING_IS_NOT_GEOMETRIC_IT_FOLLOWS_FROM_DICHOTOMIC_READOUT_ALONE_SO_THE_IMPORTED_QUANTIZATION_REDUCES_TO_B_SQUARED_EQUALS_I_AND_NO_SIGNALING_IS_EQUAL_TIME |
| `admissible_correlation_tables_probe` (J) | **which tables the encoding admits — and a correction to this PR's own first answer.** **Level A:** the representation `E_xy = ⟨ψ|A_x⊗B_y|ψ⟩` admits **exactly the unit-vector Gram matrices**, equivalently the Tsirelson–Landau–Masanes body — verified both ways (fit vs predicate **60/60**; 4000 Gram-built tables never violate, min slack **+2.6e-04**). Bodies: local **66.8%** of the cube, quantum **92.6%**, no-signaling 100%; PR box at slack **−π**. **Level B:** read off the lattice, the pair state's T-matrix x–y block is **exactly `c·I₂`** (≤ 3.1e-15) with **`c = −sin 2β`**, so with #236's U(1) settings BAM's **single-shot** tables are exactly **`E_xy = c·cos(θ_x − φ_y)`** — **0.0%** of the quantum body at maximal preparation (a measure-zero 3-parameter surface, **33.7%** of it on the Tsirelson boundary, which is why #236 saw saturation) rising to **55.2% ± 2.0%** with `β` free. **But that gap is not operational.** Shared randomness over preparations makes the reachable set the **convex hull** of the `c=1` tables, and by **linear programming** that hull is the **whole quantum body**: coverage 99.2 / **100.0** / **100.0%** at 2k/8k/32k generators, **98.7%** on 300 fresh random TLM tables with support size at most **5** — exactly the **Carathéodory bound** — and the residual failures shrink under refinement (4 → 1, worst residual 7.2e-03 → 4.2e-03 at 128k), i.e. boundary resolution, not a gap; the PR box stays outside (1.18). **The committed 'unreachable' witness decomposes into exactly 5 `c=1` cosine tables** (residual **0.0e+00**, reconstruction error **4.4e-16**), so it was never operationally unreachable — **this PR's first framing of "~45% unreachable" was a single-shot statement dressed as an operational one, and is corrected**. **The one surviving restriction:** the readout's marginals are **identically zero** (**0.000e+00**) though the state carries `cos 2β`, and unlike the correlation gap this one is **immune to mixing** (a mixture of zero-marginal behaviours has zero marginals) — so it is the whole of the probe's falsifiable content. **Method note:** generic optimizers got single-shot membership wrong in *both* directions (L-BFGS-B 48.3%, Nelder-Mead 63.3%) — a failed fit is not proof of non-representability; replaced by an exact 1-D branch/bisection test validated by 100% recovery on family-built tables. Scopes #236's "exactly sufficient" to the CHSH maximum | THE_REPRESENTATION_ADMITS_THE_GRAM_TLM_BODY_AND_SO_DOES_BAM_ONCE_MIXING_IS_ALLOWED_BECAUSE_THE_CONVEX_HULL_OF_THE_C_EQUALS_ONE_COSINE_TABLES_IS_THE_WHOLE_BODY_SO_THE_ONLY_SURVIVING_RESTRICTION_IS_IDENTICALLY_ZERO_MARGINALS |
| `detector_algebra_probe` (K) | **after #237, what remains is the detector marginals and the measurement algebra — and they are one question.** **The generated algebra, computed per pairing:** for the **fully derived** pairing (fiber U(1) setting × `σ_z` winding SG) the accessible observable set is a **single point** and the `*`-algebra it generates is exactly **`span_C{I, σ_z}`** — complex dim **2**, and **ABELIAN**. That is a *theorem-grade* explanation of the CHSH = 2 below: a commutative observable algebra admits a joint distribution, hence an LHV model, hence `CHSH ≤ 2` necessarily. Both violating pairings generate all of `M₂(C)` (dim 4, non-abelian). **Docs vs code disagree:** the documents say the setting is "the fiber-frame rotation", which on the `k = ±1` qubit is `diag(e^{iθ},e^{−iθ})` (a z-rotation), but committed `measurement_sector_probe._rot` is `exp(−iθσ_y/2)` (a y-rotation, imported **directly** from `measurement_sector_probe` and verified to **0.0e+00**; rotating the state as the code does equals conjugating the observable as this probe does, to **3.3e-16**) — and the fiber rotation **commutes exactly** with the `σ_z` winding detector (**0.00e+00**) while the y-rotation does not. **THE TRILEMMA:** max CHSH on the #206 singlet — fiber×`σ_z` (**both derived**) **2.000000**, dialable span **1**; fiber×`σ_x` (transverse assumed by #236/#237) 2.828427; y-rot×`σ_z` (the code) 2.828427. **The only pairing in which both ingredients are physically derived gives exactly the local bound — no Bell violation** — because the setting commutes with the detector and the dial does nothing. **The marginals follow the same fork:** #237's "identically zero marginals, immune to mixing" holds only for fiber×`σ_x`; under the other two the marginals are **|cos 2β|** exactly (up to **0.866**) — so that claim is **retracted as a general claim about BAM**, and #237's falsifiable content was contingent on the same undetermined choice the violation hangs on. **Both missing ingredients are the same requirement:** `σ_x` is off-diagonal between `k = ±1` and the y-rotation mixes those sectors, so BAM must supply **an operation or observable coherently connecting distinct winding sectors** — and imposing winding superselection gives max CHSH **2.000000** over 4000 draws, exactly local. Not a refutation: the violation is **located**, not overturned. Open: reconciling #208's charged-GHZ superselection with a Bell chain needing exactly the coherence it removes; whether relocating onto the transported-frame/spin doublet (#192/#197 Berger-squash) avoids it | THE_DERIVED_PAIRING_GENERATES_ONLY_THE_ABELIAN_ALGEBRA_SPAN_I_SIGMA_Z_SO_ITS_CHSH_IS_EXACTLY_TWO_BY_COMMUTATIVITY_AND_BOTH_VIOLATING_PAIRINGS_NEED_A_NONABELIAN_ALGEBRA_REQUIRING_WINDING_SECTOR_COHERENCE |
| `minimal_mixing_interaction_probe` (L) | **#238's missing interaction, tested directly instead of assumed away — it exists, it is unique, it works, and BOTH costs first reported here were overstated and are retracted.** **Uniqueness:** among mouth-localized fiber-angle potentials `λcos(2πmχ/N+φ)`, only **m = 2** connects the qubit (|⟨+1|V|−1⟩| = **0.4082**; m = 1, 3, 4 give **exactly 0**) — the qubit is `k = ±1` so the coupling must carry `Δk = 2`. The minimal missing interaction is the **second χ-harmonic at a mouth**: a **fiber-angle anisotropy**. **It is exactly the missing generator:** restricted to the qubit it is **pure `σ_x`**, coefficient **0.408248**, `I`/`σ_y`/`σ_z` zero to **1e-16**. **It works with the DERIVED detector** (no transverse readout assumed): CHSH **2.000000 → 2.828427**. *So #238's gap is not a no-go.* **CHARGE — RETRACTED.** The first version reported `‖[V₂,K]‖ = 1.414` and called this "a charge-non-conserving mouth term". That overstates it: a **prescribed external potential** breaks the winding of the **throat subsystem treated as closed**, which is not the loss of *total* charge. Extend the model with a **winding-2 carrier** and `‖[H_ext, K_total]‖ = **0.0e+00**` exactly at every truncation, with the mean-field limit reproducing the prescribed coupling **0.408248** — so the prescribed term is the **mean-field limit of a charge-conserving interaction**. The real cost is a *carrier requirement*, not a broken conservation law. **THE BELL WINDOW — RETRACTED AND REPLACED.** The first version combined an **ideal projected-qubit CHSH** with a **separate leakage proxy** via `S > 4/η − 2` — two different experiments, not a Bell test — and reported a narrow window peaking at **S = 2.13**. Computed operationally (three outcomes per mouth including **no-click**, full winding space evolved so leakage is in the dynamics, fixed assignment with **both** choices agreeing, probabilities normalizing to **1.000000000000**): **S = 2.330905** at `|t| = 1.0`, and **above the local bound at every span tested** — the violation is **broader and stronger** than first claimed. **What survives:** only a **ceiling** — 2.3309 against Tsirelson 2.8284, conceding about half the margin. **Net:** testing beat assuming, but neither cost is what was first claimed, so the case for relocating the Bell chain to the spin doublet is **weakened** — it rests on a carrier requirement and a 2.33 ceiling, not on a broken conservation law | THE_MINIMAL_MISSING_INTERACTION_IS_THE_SECOND_CHI_HARMONIC_AND_IT_WORKS_BUT_BOTH_COSTS_WERE_OVERSTATED_TOTAL_CHARGE_IS_CONSERVED_ONCE_THE_WINDING_TWO_CARRIER_IS_INCLUDED_AND_THE_OPERATIONAL_BELL_VALUE_IS_2_33_BROADLY_NOT_A_NARROW_2_13_WINDOW |
| `throat_apparatus_pointer_probe` (M) | **#239's open item executed — and the last surviving cost of the winding carrier dissolves too.** **The apparatus:** setting interaction moves `Δk = +2` while absorbing one quantum from a **winding-2 carrier**; measurement is the committed winding Stern–Gerlach `g·K_field ⊗ p̂`, which is winding-diagonal and conserves charge identically. **A CORRECTION TO #239:** winding on a discrete `N_χ = 8` fiber is a **Z₈** charge, so the conserved operator is `exp(2πiK/8)`, **not** the integer `K` — `‖[H, Z₈]‖ = **1.1e-15**` while the integer test gives **9.2–18.5**; #239's T5 got 0 only because it *excluded the wrap-around transitions*. The conclusion (total charge conserved) survives; the operator does not. **THE CEILING WAS AN ARTIFACT.** The orbit under `Δk = ±2` is the **4-cycle {+1,+3,−3,−1}**, and a winding SG deflects **∝ k**, so `k = ±3` land at their own pointer positions (4.0σ separation, overlap **4.6e-02**) — **there is no no-click outcome at all**, and #239's three-outcome model was *wrong about the detector*. Binning the complete statistics by the natural `sign(k)` rule gives CHSH **2.828028** at `|t| = 1.9` — Tsirelson (2.828427) to grid accuracy, and the best of all 16 setting-independent binnings — against #239's **2.33**, which is just one lossy binning among the 16. **Finite-carrier back-action** (carrier traced out exactly, coherent state untruncated): monotone **2.290 / 2.601 / 2.760** at `n̄ = 1/4/16` → 2.8245 mean-field; an earlier spurious non-monotonic drop was a coherent state truncated below its own mean, now fixed and reported. **No-signaling** on the four-outcome statistics: **2.2e-16**. **DECISION: the winding carrier need NOT be replaced by the spin doublet.** Across #239 and this probe *three* separate costs were reported and all three dissolved — a prescribed potential mistaken for broken charge conservation, an efficiency proxy mistaken for a Bell test, and detected channels mistaken for lost ones. What survives is **one structural requirement**: a winding-2 carrier at the mouth, populated to `n̄ ≈ 16`; whether any BAM geometry supplies one is the real open question | THE_APPARATUS_IS_CHARGE_CONSERVING_UNDER_THE_Z8_OPERATOR_AND_THE_COMPLETE_POINTER_STATISTICS_REACH_TSIRELSON_BECAUSE_THE_LEAKED_CHANNELS_ARE_DETECTED_SO_THE_2_33_CEILING_WAS_AN_ARTIFACT_AND_THE_WINDING_CARRIER_NEED_NOT_BE_REPLACED |
| `bam_spinor_spectrum_effective_action_probe` | computes BAM's actual Pin⁻ spinor instead of asserting its statistics, and **corrects the row above**. The holonomy composes with the intrinsic B2 spin structure (`T²=−I`, `η=−1`), which **swaps the spinor's moding** relative to the scalar; the two sign flips cancel and `ΔE=+π/(4C)` for the spinor too — **energetics favour untwisted in both sectors**. The twisted sector carries a zero eigenvalue (`dim ker D = 1`, `det D = 0` vs a `π/C` gap untwisted), but the index reading of that is **withdrawn on audit (T8)**: a mass or any potential lifts it, the mode is the constant function (overlap 1.000000000000), and the **index is 0** — `dim ker` is not an index, and the 1D cycle has no monopole or chirality grading (unlike #195's S²+monopole). Net: energy explains the bosonic QCD sector; **why matter is twisted is reopened**. Spectrum via the repo's SUSY square root, `O(1/n²)`; method note: a central-difference Dirac operator is unusable (doubling gives a spurious second zero mode) | THE_BAM_SPINOR_SWAPS_THE_MODING_SO_ENERGETICS_FAVOUR_UNTWISTED_IN_BOTH_SECTORS_AND_THE_KERNEL_IS_A_BOUNDARY_CONDITION_ZERO_MODE_NOT_AN_INDEX_SO_MATTERS_TWISTED_SECTOR_IS_OPEN | THE_TWIST_SECTOR_IS_SELECTED_ENERGETICALLY_AND_THEN_TOPOLOGICALLY_FROZEN_AT_NUCLEATION_SIGN_RULE_CORRECTED_BY_THE_SPINOR_PROBE |
...
## Cross-references
  ...
  - `docs/bam_effective_action_scaffold_research_plan.md` *(absent)* — the original
    5-barrier map.
  - `docs/topological_discrete_sector_research_plan.md` — B1 + B2.
  - `experiments/closure_ledger/hard_wall_boundary_verification.py` — B3
    (no separate research-plan doc; the derivation is in this file and in
    the B3 section above).
  ...
  - `docs/glueball_closed_flux_loop_research_plan.md` — PR #100, uses
    ...
    `M²` that interleaves the orientable one (≈2× the states).
    **Corrected by PR #229:** the `½` is confirmed (computed three ways,
    topological), so the `πσ` level shift stands, but the two towers were
    given a common intercept `M₀²` — antiperiodic moding shifts the zero
    point by `+π/(4C)` per polarization, so the Möbius ground state is
    **not** `M₀² + πσ` and its absolute masses are open.
     Since
