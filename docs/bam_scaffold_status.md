...
| `twist_sector_selection_probe` | what selects the twist sector (the arc's last open item): **energetics select, topology freezes.** Sectors differ by `π/(4C)` per d.o.f. (1e-12), with the **sign set by the link field's statistics** — bosonic ⟹ untwisted favoured, fermionic ⟹ twisted favoured. But `W` cannot relax: changing it drives the link amplitude through zero, severing the cycle (`b₁`:1→0) — the #175 gate; `W` is exactly deformation-invariant (200 re-weightings, zero-mode gap never closes). One Casimir sign gets **both** of the program's Möbius sectors right (QCD phonon bosonic ⟹ excitations above ground; Pin⁻ spinor fermionic ⟹ matter in the twisted sector) | THE_TWIST_SECTOR_IS_SELECTED_ENERGETICALLY_WITH_A_SIGN_SET_BY_THE_LINK_FIELD_STATISTICS_AND_THEN_TOPOLOGICALLY_FROZEN_AT_NUCLEATION |
