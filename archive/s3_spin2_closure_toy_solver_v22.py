"""
s3_spin2_closure_toy_solver_v22.py
====================================
Full validated solver — v22.
...
---------------------------------------------------------
  ...
  TT bulk propagation (v21): t_plus/t_cross come from a genuine
  source→propagate→evaluate chain via propagated_tt_amplitudes(n,m,...):
      z_prop = A * conj(Y_nm(src)) * exp(i*(wn*Deta-beta-wh)) * Y_nm(dst)
  The source mouth excites the (n,m) eigenmode; it propagates around the S³
  cavity with the resonance phase; it is sampled at the destination mouth.
  Also included: tt_shell_propagator_s3 (full same-n shell Green function).
  tt_mode_z feeds bulk_amp; rank_score = score*(1+|tt_mode_z|) sorts the table.
...
TTScanMode  = Literal["diag_m", "shell_sum"]  # v22: selects diagonal vs full-shell TT propagation
...
print("v22: computing Tangherlini eigenspectrum ...", flush=True)
_L_MAX_DEFAULT = 27
...
def tt_to_vector_mix(
    H_bulk: Tensor3x3, *,
    chi_dst: float = pi/2,
    eps_tt_mix: float = 0.0,
) -> Tuple[float, float, float]:
    """
    Optional explicit TT→vector mixing term (v22).

    The standard projector Π₂→₁ annihilates the TT block exactly.
    This function adds a small controlled leakage from the TT sector
    into the transverse vector slots (v1, v2) only — the axial slot
    remains zero so the main kinematic signal is not polluted.

        mix = (0, eps_tt_mix * t_plus, eps_tt_mix * t_cross)

    This is an EXPLICIT NEW HYPOTHESIS beyond the current projector
    geometry, not disguised as existing physics.  Default eps_tt_mix=0
    recovers the exact projector with zero mixing.
    Physical motivation: anisotropy, off-axis mouth orbits, or curvature-
    induced coupling could produce small TT→vector leakage at this scale.
    """
    if eps_tt_mix == 0.0:
        return (0.0, 0.0, 0.0)
    dc = decompose_tensor_about_throat(H_bulk, chi=chi_dst)
    return (0.0, eps_tt_mix * dc["t_plus"], eps_tt_mix * dc["t_cross"])
...
def dressed_mouth_response_from_tensor(
    ...
    axis: Tuple[float,float,float] = E_PAR,
    spectrum: str = "tensor_s3",
    eps_tt_mix: float = 0.0,
) -> Tuple[float,float,float]:
    """
    Two-step 5→3 with spectrum-aware response dressing and optional TT mixing.
    With eps_tt_mix=0 (default): exact projector, TT sector annihilated.
    With eps_tt_mix>0: adds small TT→vector leakage via tt_to_vector_mix.
    """
    A   = tensor_to_vector_projector(H_bulk, axis)
    mix = tt_to_vector_mix(H_bulk, chi_dst=chi_dst, eps_tt_mix=eps_tt_mix)
    A_eff = tuple(A[i] + mix[i] for i in range(3))  # type: ignore
    lam = lambda_geom(n, chi_dst, R3, spectrum=spectrum)
    H_k = (plus_tensor_operator(chi_dst) if spectrum == 'tensor_s3'
           else connection_tensor_operator(chi_dst))
    HA  = matvec3(H_k, A_eff)
    return tuple(kappa_vec * ((1.0 - lam) * A_eff[i] + lam * HA[i]) for i in range(3))  # type: ignore
...
def tt_propagated_mode_amplitude(
    n: int, m: int, *,
    chi_src: float, phi_src: float,
    chi_dst: float, phi_dst: float,
    delta_eta_val: float,
    beta: float,
    wormhole_phase: float,
    envelope: float,
    gamma_tt: float = 0.0,
    spectrum: str = "tensor_s3",
) -> complex:
    """
    Diagonal propagated TT field for a single (n,m) cavity eigenmode.

    Physics: the source mouth excites the (n,m) TT mode with amplitude
    proportional to conj(Y_nm(x_src)).  The mode propagates around the S³
    cavity, picking up the resonance phase (omega_n * delta_eta - beta - wh)
    and damping gamma_tt * delta_eta.  At the destination mouth, the field
    amplitude is Y_nm(x_dst).

        z_prop = envelope * conj(Y_nm(src)) * exp(-gamma*Deta + i*(wn*Deta - beta - wh))
                          * Y_nm(dst)

    This replaces the local destination-only overlap of mode_derived_tt_amplitudes,
    giving an actual source→propagate→evaluate chain rather than a boundary sample.
    Returns 0j for vector/gauge spectra (no TT block) and for j=n-1 < 2.
    """
    if spectrum != "tensor_s3":
        return 0.0j
    y_src = tt_mode_overlap_s3(n, m, chi_src, phi_src)
    y_dst = tt_mode_overlap_s3(n, m, chi_dst, phi_dst)
    if abs(y_src) < 1e-15 and abs(y_dst) < 1e-15:
        return 0.0j
    wn   = omega_tensor_s3(n, 1.0)   # R=1 in solver natural units
    prop = cmath.exp(
        (-gamma_tt * delta_eta_val)
        + 1j * wrap_to_pi(wn * delta_eta_val - beta - wormhole_phase)
    )
    return envelope * y_src.conjugate() * prop * y_dst


def propagated_tt_amplitudes(
    n: int, m: int, *,
    chi_src: float, phi_src: float,
    chi_dst: float, phi_dst: float,
    delta_eta_val: float,
    ...
    """
    (t_plus, t_cross, tt_mode_z) from the propagated TT field.

    Replaces mode_derived_tt_amplitudes in exact_resonance_from_quantum_numbers.
    Returns Re(z), Im(z), z where z = tt_propagated_mode_amplitude(...).
    """
    z = tt_propagated_mode_amplitude(
        n, m,
        chi_src=chi_src, phi_src=phi_src,
        chi_dst=chi_dst, phi_dst=phi_dst,
        delta_eta_val=delta_eta_val,
        beta=beta,
        wormhole_phase=wormhole_phase,
        envelope=envelope,
        gamma_tt=gamma_tt,
        spectrum=spectrum,
    )
...
def tt_shell_propagator_s3(
    ...
    """
    Full same-n shell propagator: sum over all helicities mu = -j..j.

        G_n(dst, src; Deta) = prop * sum_mu conj(Y_nm(src)) * Y_nm(dst)

    This is the Green function for the n-th TT shell of the S³ cavity.
    For the diagonal single-m resonance scan, prefer propagated_tt_amplitudes
    (which fixes mu=m to the scanned quantum number).
    Use this for post-scan shell-coherence analysis or sum-over-m averaging.
    """
    j = n - 1
    wn   = omega_tensor_s3(n, 1.0)
    prop = cmath.exp(
        (-gamma_tt * delta_eta_val)
        + 1j * wrap_to_pi(wn * delta_eta_val - beta - wormhole_phase)
    )
    return prop * sum(
        tt_mode_overlap_s3(n, mu, chi_dst, phi_dst)
        * tt_mode_overlap_s3(n, mu, chi_src, phi_src).conjugate()
        for mu in range(-j, j + 1)
    )
...
def mouth_response_operator(*, delta_eta_val, dphi_src, dphi_dst,
    ...
    if H_bulk is not None and s2 > 1e-12:
        F  = dressed_mouth_response_from_tensor(
            ...
            n=n, R3=R3, axis=E_PAR, spectrum=spectrum, eps_tt_mix=eps_tt_mix)
...
print("v22: calibrating drive_scale for C5 (throat-source ~ α/π) ...", flush=True)
...
def exact_resonance_from_quantum_numbers(n, N, kind, branch, wc, r_src, r_dst, m, sigma, k,
    *, R3=1.0, beta=0.0, spectrum="tensor_s3",
    ...
    alpha_q_table=None,
    tt_scan_mode: TTScanMode = 'diag_m',
    eps_tt_mix: float = 0.0,
) -> Resonance:
    ...
    resp = mouth_response_operator(delta_eta_val=De, dphi_src=ds, dphi_dst=dd,
        ...
        beta_wh=beta_wh, beta_resp=beta_resp, chi_src=chi_src, chi_dst=chi_dst,
        strength_scale=strength_scale, spectrum=spectrum)
    # Step 2: construct H_bulk from PROPAGATED TT field (v22).
    #   tt_scan_mode='diag_m'  — diagonal single-m propagation (selected helicity channel)
    #   tt_scan_mode='shell_sum' — full same-n shell Green function (all mu summed)
    _s2 = sin(chi_dst)**2
    _sigma_bulk = resp.current_vec[0] / max(_s2, 1e-12)
    phi_src_geom = wrap_to_pi(ds + resp.hopf_phase_src)
    phi_dst_geom = wrap_to_pi(dd + resp.hopf_phase_dst + beta_resp)
    if spectrum != 'tensor_s3':
        tt_mode_z = 0.0j
        _t_plus, _t_cross = 0.0, 0.0
    elif tt_scan_mode == 'diag_m':
        _t_plus, _t_cross, tt_mode_z = propagated_tt_amplitudes(
            n, m,
            chi_src=chi_src, phi_src=phi_src_geom,
            chi_dst=chi_dst, phi_dst=phi_dst_geom,
            delta_eta_val=De, beta=beta, wormhole_phase=wh,
            envelope=resp.strength, gamma_tt=0.0, spectrum=spectrum,
        )
    else:  # shell_sum
        tt_mode_z = resp.strength * tt_shell_propagator_s3(
            n,
            chi_src=chi_src, phi_src=phi_src_geom,
            chi_dst=chi_dst, phi_dst=phi_dst_geom,
            delta_eta_val=De, beta=beta, wormhole_phase=wh, gamma_tt=0.0,
        ) / max(2 * n - 1, 1)  # mild shell-size normalization
        _t_plus, _t_cross = tt_mode_z.real, tt_mode_z.imag
    ...
    resp = mouth_response_operator(delta_eta_val=De, dphi_src=ds, dphi_dst=dd,
        ...
        beta_wh=beta_wh, beta_resp=beta_resp, chi_src=chi_src, chi_dst=chi_dst,
        strength_scale=strength_scale, spectrum=spectrum,
        H_bulk=H_bulk_res, eps_tt_mix=eps_tt_mix)
    src = throat_mode_sourcing(resp.response_vec, resp.phase,
        bulk_frequency=wn, delta_eta_val=De,
                                            ...
                                            for kk in range(kc-4, kc+5):
                                                try:
                                                    sol = exact_resonance_from_quantum_numbers(
                                                        ...
                                                        tt_scan_mode=tt_scan_mode,
                                                        eps_tt_mix=eps_tt_mix,
                                                    )
                                                except (ValueError, ZeroDivisionError): continue
...
def compare_tt_scan_modes(**scan_kwargs) -> Dict:
    """
    Run both 'diag_m' and 'shell_sum' TT scan modes and return side-by-side results.

    diag_m:    one selected helicity channel (n,m) propagated through the cavity.
    shell_sum: same-n shell Green-function propagation, all mu summed and normalized.

    Returns {
        "diag":  {"resonances": [...], "families": [...]},
        "shell": {"resonances": [...], "families": [...]},
    }
    Usage: cmp = compare_tt_scan_modes(spectrum="tensor_s3", n_max=6, ...)
    """
    diag  = enumerate_exact_resonances(tt_scan_mode="diag_m",   **scan_kwargs)
    shell = enumerate_exact_resonances(tt_scan_mode="shell_sum", **scan_kwargs)
    return {
        "diag":  {"resonances": diag,  "families": group_interference_families(diag,  min_members=2)},
        "shell": {"resonances": shell, "families": group_interference_families(shell, min_members=2)},
    }
...
def run_validation_suite() -> Dict:
    ...
    print(SEP)
    print("v22 FULL VALIDATION SUITE — external review")
    print(SEP)
    ...
    print(f"  n=18 from λ=α/(3π); r0 from P=α/π")
    print(f"  drive_scale calibrated so bounded strength=|q/Q|/(|q/Q|+1)=α/π")
...
def demo() -> None:
    # Side-by-side diagonal-m vs shell-sum TT scan comparison
    _KWARGS = dict(
    ...
    cmp = compare_tt_scan_modes(**_KWARGS)
    print("\n=== DIAGONAL-m TOP RESONANCES ===")
    print(format_table(cmp['diag']['resonances'], limit=8))
    print("\n=== SHELL-SUM TOP RESONANCES ===")
    print(format_table(cmp['shell']['resonances'], limit=8))
    print("\n=== DIAGONAL-m FAMILIES ===")
    print(format_family_table(cmp['diag']['families'], limit=6))
    print("\n=== SHELL-SUM FAMILIES ===")
    print(format_family_table(cmp['shell']['families'], limit=6))
    # eps_tt_mix sensitivity: show how small TT->vector leakage shifts top scores
    print("\n=== EPS_TT_MIX SENSITIVITY (eps=0 vs eps=0.03) ===")
    for eps in (0.0, 0.03):
        exact_e = enumerate_exact_resonances(eps_tt_mix=eps, **_KWARGS)
        fams_e  = group_interference_families(exact_e, min_members=2)
        r0 = exact_e[0]
        print(f"  eps={eps:.2f}: top rank_score={r0.rank_score:.4e}  "
              f"n={r0.n} m={r0.m}  families={len(fams_e)}")
