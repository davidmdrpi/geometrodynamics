...
T4  THE TRANSLATION MODE DOES NOT COUPLE.  The ℓ = 1 control done on the mutual
...
T5  THE SAME TRAP, IN THE AREA.  The second variation of the area of
...
T6  AND TRANSLATION INVARIANCE HOLDS ANYWAY.  The rigid displacement carries
    induced ℓ = 0 and ℓ = 2 pieces; the exact displaced sphere is
    area-preserving to 4e-16 at every d.

T7  THE AREA COST IS THE EXACT RATIONAL at every ℓ checked.

T8  THE COUPLING IS SCREENED.  (b/a)^ℓ, a factor of 544 from ℓ = 1 to ℓ = 8 at
    b/a = 0.4 — the modes carrying a spin-2 signal are the ones separation
    screens hardest.

T9  THE SHEAR RESPONSE IS AN INPUT.  A perfect-fluid shell resists area change
...
def t4_the_translation_mode_does_not_couple() -> dict:
    ...
    return {"name": "T4_the_translation_mode_does_not_couple", **r,
...
def t5_the_pure_dipole_is_not_a_translation() -> dict:
    ...
    return {"name": "T5_the_pure_dipole_is_not_a_translation", **r,
            "pass": bool(r["the_closed_form_is_confirmed"]
...
def t6_translation_invariance_is_exact() -> dict:
    ...
    return {"name": "T6_translation_invariance_is_exact", **r,
...
def t7_the_area_cost_is_the_exact_rational() -> dict:
    ...
    return {"name": "T7_the_area_cost_is_the_exact_rational", **r,
...
def t8_the_coupling_is_screened_by_separation() -> dict:
    ...
    return {"name": "T8_the_coupling_is_screened_by_separation", **r,
...
def t9_the_shear_response_is_an_input() -> dict:
    ...
    return {"name": "T9_the_shear_response_is_an_input", **r,
...
def run_probe() -> dict:
    ...
    t2, t4, t5, t8 = tests[1], tests[3], tests[4], tests[7]
    ...
    if all(t["pass"] for t in tests):
        ...
        verdict = (
            ...
            "l = 0. The prefactor is the angular-Laplacian eigenvalue, so the "
            "l = 0 decoupling IS that zero eigenvalue. This is the Newtonian "
