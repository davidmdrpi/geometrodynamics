...
**6/6 checks pass — the branch is narrowed, not closed.**
...
Added after review. Three earlier docstrings credited this to the `S⁴_R` slice being maximally symmetric. Wrong reason, and it understates the result: in `D = 5` the spatial block of `H^a_b` is the **4D Gauss–Bonnet (Euler) tensor** of `h₄`, identically zero because Gauss–Bonnet is topological in `D = 4`.

| metric | `max|H^i_j|` at `h = 5e-4` | ratio over a 4× refinement |
|--|--|--|
| throat, s = 0.9 (ultrastatic, NOT max. symmetric) | `1.94e-06` | `16.0` |
| throat, at the neck s = 0 | `5.10e-05` | `16.0` |
| generic lumpy 4-slice (no symmetry at all) | `6.43e-10` | `16.0` |
| CONTROL: same slice, nonconstant lapse | `2.30e-02` | `1.0` |

> Breaking the product with a nonconstant lapse leaves a spatial block of 2.30e-02 that does NOT shrink under refinement, so the zeros above are a result rather than a property of the instrument.

> **Gauss-Bonnet cannot touch the pressures anywhere on this geometry. The entire alpha-dependence lands in the density, in the throat as well as in the exterior.**
...
| the vanishing spatial block H^i_j = 0 is due to the slice being maximally symmetric | **NO -- IT IS THE 4D EULER TENSOR** | H^i_j vanishes (ratio 16 under a 4x step reduction) for the throat slice and for a generic lumpy 4-slice with no symmetry; a nonconstant-lapse control stays at 2.3e-2 and does not shrink. So Gauss-Bonnet cannot touch the pressures ANYWHERE on this geometry |
