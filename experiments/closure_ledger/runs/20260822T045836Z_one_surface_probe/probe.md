...
| T9 | *** the offset turns cancellation OFF, not interference on *** | PASS |
...
| `5` | `0.2000π` | `2.0000` | `0.7915` | `1.522` | `0.8000` | yes |
...
At fixed energy the highest mode that still spans the gap is `m = 7`.
...
Apart, the total is `1.012–1.017×` **one** contribution, and it peaks exactly where a single contribution does.

> not interference -- the offset turns the CANCELLATION off by pulling the two fronts apart, and what is left is two nearly independent dents in one surface
